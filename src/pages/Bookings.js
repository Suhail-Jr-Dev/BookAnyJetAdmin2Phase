...
const Bookings = () => {
    ...
    const sortedResult = async () => {
        try {
            ...
            const payload = {
                from: formatDate(fromDate),  // Ensure it’s in dd-mm-yyyy format
            ...
            if (resObj.data && resObj.data.data) {
                let arrayOfData = resObj.data.data.map((element, index) => {
                    return {
                        ...
                        type: element.type,
                        passengers: element.passengers,
                        date: element.date, // Format the date here
                        action: element._id,
                    };
                });
                setData(arrayOfData);
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 250, // Set the width to 300px or any desired value
        }
        ,
        {
            title: 'Phone',
       ...
       width:80
        ...
        },
        {
            ...
            dataIndex: 'action',
            key: 'action',
            render: (text, record) => <a onClick={() => deleteHandler(record.key)}>Delete</a>,
        },
    ];
    ...
    return (
        <div>
            <h1 className='p-3 m-4 text-4xl'>Manage All Bookings Here.</h1>

            <div id="datePicker" className='flex items-center justify-around h-16 gap-2 m-8'>
                <form method="post" className='flex items-center justify-center w-[50%] h-[100%] text-1xl font-bold cursor-pointer rounded-lg overflow-hidden' action='/api/admin/sorted cursor-pointer'>
                    <input
                        type="date"
                        name="from"
                        id="from"
                        ref={fromDateRef}
                        className='w-[100%] h-[100%] p-3 outline-none cursor-pointer transition-all duration-500 ease-in-out border'
                    ...
                    <input
                        ...
                        id="to"
                        ref={toDateRef}
                        className='w-[100%] h-[100%] p-3 outline-none cursor-pointer border'
                        onChange={(e) => {
                ...
                {(fromDate && toDate) && (
                    <button
                        className='p-4 font-bold border shadow-md outline-none rounded-xl bg-zinc-300'
                ...
                <input
                    ...
                    ref={toDateRef}
                    placeholder='Search'
                    className='w-[20%] h-[100%] p-3 outline-none cursor-pointer m-3 rounded-lg border'
